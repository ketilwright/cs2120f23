...
-- match on n = 0. Don't apply f at all: just return a
| f, a, 0 => a --?
-- match on n ≠ 0. recurse on f with the value of f(a)
...
one to create an empty list of α values, and
...
type builder is defined (simplified just a tad).
-/
...
-/
-- finally a real case of using unit and Sum
...
| 0 => [0]
| n + 1 => (n + 1) :: countdown n -- or Nat.succ n instead of n + 1
...
#eval list_rev ["we", "hold", "these", "truths", "to", "be"]
/-!
