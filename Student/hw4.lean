...
Doing so will really solidify your understanding.
-/
...
-- Here:
def prod_com_reverse {α β} : β × α → α × β
| (b, a) => (a, b)
...
That expression, in turn uses an explicit match
...
that does it. Call it sum_comm_reverse.
-/
...
def tSumStrNat: Type := String ⊕ Nat
def tSumNatStr: Type := Nat ⊕ String
--def oSumStrNat: tSumStrNat := "foo" ⊕ 42
def sn: String ⊕ Nat := Sum.inl "hello"
#eval sum_comm_reverse sn -- Sum.inr "hello"
-- Not clear how (or if) this would work: Not type for β
-- #eval sum_comm_reverse (Sum.inl "hello")
...
#check sum_assoc -- {α β γ : Type} → α ⊕ β ⊕ γ → (α ⊕ β) ⊕ γ
